 ...
 class PorterStemmer {
		...
		int length(string line){
			...
			// cout<<"inside the porterstemmer\n";
			wordArray=word;
			if (length(word) <= 2) return word;
			//wordArray = word.ToCharArray();
			stemIndex = 0;
			endIndex = length(word) - 1;
			Step1();
			Step2();
			Step3();
			Step4();
			...
			Step6();
			int size = endIndex + 1;
			wordArray=wordArray.substr(0,size+1);
			//cout<<"wordArray  "<<wordArray<<endl;
			return wordArray;
		}
		// Step1() gets rid of plurals and -ed or -ing.
		private: void Step1() {
			//cout<<"inside the step1\n";
		...
		private: void Step6() {
			//cout<<"inside the step6\n";
			stemIndex = endIndex;
			if (wordArray[endIndex] == 'e') {
				int a = MeasureConsontantSequence();
		...
		private: bool EndsWith(string s) {
			int n = length(s);
			int index = endIndex - n + 1;
			if (index < 0) return false;
			for (int i = 0; i < n; i++) {
				if (wordArray[index + i] != s[i]) return false;
			}
			stemIndex = endIndex - n;
			return true;
		}
